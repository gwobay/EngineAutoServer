...
/*        try
	...
	protected boolean sendByte(byte[] b, int from, int length)
	{
				
		try {
			if (mOut==null) mOut=mSocket.getOutputStream();
			
			mOut.write(b, from, length);
			mOut.flush();			
		} catch (IOException e){
			writeFlag=BAD_STREAM; 
			return false;
	...
	{
		byte[] textByte=data.getBytes();
		return sendByte(textByte, 0, textByte.length);
	}
	
	public boolean sendData(byte[] textByte)
	{		
		return sendByte(textByte, 0, textByte.length);
	}
	
	public SimpleSocket(Proxy arg0) {
		super(arg0);
		// TODO Auto-generated constructor stub
	}

	public SimpleSocket(SocketImpl impl) throws SocketException {
		super(impl);
		// TODO Auto-generated constructor stub
	}


	public SimpleSocket(InetAddress address, int port) throws IOException {
		super(address, port);
		// TODO Auto-generated constructor stub
	}

	public SimpleSocket(String host, int port, InetAddress localAddr, int localPort)
			throws IOException {
		super(host, port, localAddr, localPort);
		// TODO Auto-generated constructor stub
	}

	public SimpleSocket(InetAddress address, int port, InetAddress localAddr,
